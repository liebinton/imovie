var express = require('express');
var router = express.Router();
var index = require("../controller/index.js");

router.get('/',index.showindex);
router.get('/movie',index.showmovie);
router.get('/login',index.login);
router.get('/signin',index.signin);
router.post('/login_check',index.login_check);
router.post('/signin_check',index.signin_check);
router.get('/list',index.get_list);
router.get('/user',index.user_info);
router.get('/show_change_page',index.show_change_page);
router.post('/userinfo',index.change_userinfo);
router.post('/comments',index.post_comment);
router.get('/ticket',index.ticket_list);
router.get('/order',index.order_page);
router.post('/buy',index.buy_ticket);
router.get('/all',index.show_all_page);
router.get('/ticket_manage',index.show_ticket_manage);
router.get("/admin_login_page",index.admin_login_page);
router.post('/admin_login',index.admin_login);
router.get("/ticket_delete",index.admin_delete_ticket);
router.get("/add_ticket",index.admin_add_ticket);
router.post("/add_ticket",index.do_admin_add_ticket);
// router.get('/undefined',index.show_err);
module.exports = router;
